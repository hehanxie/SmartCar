...
{ 
 ...
 if(pixchange_flag==0)
  {
    pixchange_flag=1;
    img_extract(*Img,(unsigned char*)Pix_Data_prev,600);//图像解压
    LPLD_DMA_LoadDstAddr(DMA_CH0,(unsigned char*)Pix_Data);
  }
  else
  {
    pixchange_flag=0;
    img_extract(*Img,(unsigned char*)Pix_Data,600);//图像解压
    LPLD_DMA_LoadDstAddr(DMA_CH0,(unsigned char*)Pix_Data_prev);
  }
...
void ov7725_init(void)
{
  ...
  SCCB_WriteByte(OV7725_HOutSize,0x14);//80
  SCCB_WriteByte(OV7725_VOutSize,0x1E);//60 
